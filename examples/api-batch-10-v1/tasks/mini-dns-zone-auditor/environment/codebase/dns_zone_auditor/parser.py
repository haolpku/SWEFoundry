"""Starter parser for a deliberately small DNS master-file subset.

This version is intentionally useful but incomplete: it preserves owner-name
case, so canonical case-insensitive behavior is supplied by the step-1 solution.
"""
...
def parse_zone(text: str, origin: str) -> list[DnsRecord]:
    ...
    for line in _logical_lines(text):
        ...
        if parts[0].upper() == "$TTL":
            ...
            continue
        idx = 0
        if parts[idx].upper() in _SUPPORTED or parts[idx].upper() == "IN" or parts[idx].isdigit():
