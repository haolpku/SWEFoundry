"""Complete oracle: deterministic atomic manifest install audit implementation."""
...
def audit_install(root: str, manifest_path: str, log_path: str) -> InstallAuditReport:
    ...
    rollback = plan_rollback(root, log_path) if os.path.exists(log_path) else RollbackPlan((), ())
    return InstallAuditReport(tuple(recovered), tuple(operations), rollback)
