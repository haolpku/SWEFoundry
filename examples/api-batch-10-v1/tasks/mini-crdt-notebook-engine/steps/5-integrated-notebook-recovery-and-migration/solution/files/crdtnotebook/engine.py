...
def merge_notebook_text(ops: list[NotebookOp]) -> list[NotebookCell]:
    ...
    walk(None)
    all_cells = sorted(set(visible) | set(dead) | set(titles))
    return [NotebookCell(cell, "".join(visible.get(cell, [])), titles.get(cell, ""), tuple(sorted(dead.get(cell, [])))) for cell in all_cells]
...
def _snapshot_records(snapshot: str | None) -> tuple[list[dict[str, Any]], bool, bool]:
    if snapshot is None or snapshot.strip() == "":
        return [], False, False
    try:
        value = json.loads(snapshot)
    except json.JSONDecodeError as exc:
        raise NotebookParseError(f"snapshot: {exc.msg}") from exc
    if isinstance(value, list):
        return value, True, True
    if not isinstance(value, dict):
        raise NotebookParseError("snapshot must be an object or operation list")
    migrated = value.get("schema_version") not in {"1.0", 1}
    records = value.get("operations", value.get("ops", value.get("journal", [])))
    if not isinstance(records, list) or not all(isinstance(item, dict) for item in records):
        raise NotebookParseError("snapshot operations must be a list of objects")
    return records, True, migrated


def audit_crdt_notebook(snapshot: str | None, journal: str, peer_frontiers: dict[str, list[str]]) -> CrdtNotebookAuditReport:
    snap_records, recovered, migrated = _snapshot_records(snapshot)
    journal_records = _load_records(journal)
    canonical = json.dumps(snap_records + journal_records, sort_keys=True, separators=(",", ":"))
    ops = tuple(parse_notebook_ops(canonical))
    problems = tuple(validate_notebook_dag(list(ops)))
    cells: tuple[NotebookCell, ...]
    if problems:
        cells = ()
    else:
        cells = tuple(merge_notebook_text(list(ops)))
    plan = plan_notebook_compaction(list(ops), peer_frontiers)
    return CrdtNotebookAuditReport("1.0", ops, problems, cells, plan, recovered, migrated)
